...
def create_base_prompt(question: str, history: str, instructions: list[str]) -> str:
...
def create_base_prompt(question: str, history: str) -> str:
    prompt = f"Question: {question}\n\n"
    prompt += "Below are the history provided by the user."
    prompt += f"{history}\n\n"
    prompt += (
        "\nAnalyze the provided history and answer the question based on the following guidelines:\n"
        "1. Extract and organize relevant information about people mentioned in the history\n"
        "2. Focus on relationships, events, and interactions between individuals\n"
        "3. If multiple instances of the same person are mentioned, consolidate the information\n"
        "4. Provide a clear, structured response that directly addresses the question\n"
        "5. If the information is not available in the history, clearly state it\n"
        "6. Do not make assumptions beyond what is explicitly stated in the history"
    )
    return prompt
